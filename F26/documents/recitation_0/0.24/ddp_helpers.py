...
def train():
    ...
    rank = int(os.environ["RANK"]) #What process am I? (global)
    local_rank = int(os.environ["LOCAL_RANK"]) #Which GPU is mine?
    world_size = int(os.environ["WORLD_SIZE"]) #How many processes are there?

    # initialize process group
    torch.cuda.set_device(local_rank) #which GPU to use
    #establish communication group between all processes
    dist.init_process_group(backend="nccl", device_id=torch.device(f"cuda:{local_rank}"))

    # Let one process perform operations that only need to happen once
    if rank == 0:
